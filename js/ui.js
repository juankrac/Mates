// ============================================================
// UI: MODALES, HUD, LOGROS, MISIONES, JEFE Y FLECHAS
// ============================================================
...
// ============================================================
// EMPEZAR A PRACTICAR (pasa de teoria a ejercicio)
// ============================================================
...
function renderExercise() {
  ...
  const idx = session.ejercicioIndexPorTema[session.currentTopic.id] || 0;
  const ej = session.currentTopic.ejercicios[idx];
  if (!ej) return;
  ...
  document.getElementById('exercise-counter').textContent = idx + 1;
  document.getElementById('exercise-total').textContent = session.currentTopic.ejercicios.length;
...
// ============================================================
// NAVEGAR ENTRE EJERCICIOS (flechas)
// ============================================================
export function nextExercise() {
  if (!session.currentTopic) return;
  const total = session.currentTopic.ejercicios.length;
  const idx = session.ejercicioIndexPorTema[session.currentTopic.id] || 0;
  session.ejercicioIndexPorTema[session.currentTopic.id] = (idx + 1) % total;
  renderExercise();
  Sounds.change();
}
...
export function prevExercise() {
  ...
  const total = session.currentTopic.ejercicios.length;
  const idx = session.ejercicioIndexPorTema[session.currentTopic.id] || 0;
  session.ejercicioIndexPorTema[session.currentTopic.id] = (idx - 1 + total) % total;
  renderExercise();
  Sounds.change();
}
...
export function checkAnswer() {
  ...
  let isCorrect = false;
  const uN = parseFloat(user);
  const cN = parseFloat(correct);

  if (!isNaN(uN) && !isNaN(cN)) {
    isCorrect = Math.abs(uN - cN) < 0.001;
  } else {
  ...
  if (!isCorrect && correct.indexOf('/') !== -1) {
    ...
    if (!isNaN(n) && !isNaN(d) && d !== 0 && !isNaN(uN)) {
      isCorrect = Math.abs(uN - n / d) < 0.001;
    }
  }
  if (!isCorrect && user.indexOf('/') !== -1) {
    ...
    const n = parseFloat(p[0]);
    const d = parseFloat(p[1]);
    if (!isNaN(n) && !isNaN(d) && d !== 0 && !isNaN(cN)) {
      isCorrect = Math.abs(n / d - cN) < 0.001;
    }
  }
...
export function updateMissionsBadge() {
  ...
  const pend = MISSION_TEMPLATES.filter(m =>
    session.dailyData.completed.indexOf(m.id) !== -1 &&
    session.dailyData.rewarded.indexOf(m.id) === -1
  );
  if (pend.length > 0) {
...
export function renderMissions() {
  ...
  MISSION_TEMPLATES.forEach(m => {
    ...
    let boton = '';
    if (done && !rewarded) {
      boton = ' - <button class="mission-claim-btn" data-mission="' + m.id + '">RECLAMAR!</button>';
    } else if (rewarded) {
      boton = ' - OK';
    }
...
export function isBossUnlocked() {
  return session.totalStars >= 40;
}

export function openBossModal() {
  if (!isBossUnlocked()) {
    alert('Necesitas 40 estrellas para desafiar al jefe.');
    return;
  }
  if (!state.nearBoss) {
    alert('Acercate a la cueva del jefe.');
    return;
  }
...
export function renderBossQuestion() {
  if (session.bossIndex >= BOSS_QUESTIONS.length) {
    finishBoss(true);
    return;
  }
...
export function tryOpenNearbyHouse() {
  ...
  const zoneUnlocked = session.totalStars >= ZONES[state.nearHouse.zone].unlockStars;
  if (!zoneUnlocked) {
...
export function bindUI() {
  ...
  document.getElementById('achievements-btn').addEventListener('click', () => {
    ...
    session.achievements.forEach(a => {
      ...
      d.innerHTML =
        ...
        '<div class="ach-info">' +
          '<div class="ach-name">' + a.name + (a.unlocked ? ' OK' : ' - bloqueado') + '</div>' +
  ...
  document.getElementById('missions-modal').addEventListener('click', e => {
    if (e.target === document.getElementById('missions-modal')) {
      document.getElementById('missions-modal').classList.remove('show');
    }
  });
  ...
  // --- Reanudar audio tras el primer click ---
