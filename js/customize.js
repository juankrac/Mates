...
// ============================================================
// INICIALIZAR LA ESCENA DE VISTA PREVIA
// ============================================================
export function initPreviewScene() {
  ...
  refs.previewRenderer = new THREE.WebGLRenderer({ canvas: canvas, antialias: true });
...
// ============================================================
// CONSTRUIR EL PERSONAJE DE LA VISTA PREVIA
// ============================================================
export function buildPreviewCharacter() {
  ...
  const head = new THREE.Mesh(
    ...
    toonMat(appearance.skinColor)
  );
  head.position.y = 2;
  head.castShadow = true;
  refs.previewCharacter.add(head);
  addOutline(head, 1.08);

  const bodyColor = (appearance.outfitColor === '0xrainbow' || appearance.outfitColor === 'rainbow')
    ? 0xff6b6b
    : (parseInt(appearance.outfitColor, 16) || 0x3d5a99);
  ...
  refs.previewCharacter.add(body);
  ...
  refs.previewCharacter.add(armL);
  addOutline(armL, 1.15);

  const armR = armL.clone();
  armR.position.x = 0.65;
  refs.previewCharacter.add(armR);
  ...
  refs.previewCharacter.add(legL);
  addOutline(legL, 1.15);

  const legR = legL.clone();
  legR.position.x = 0.22;
  refs.previewCharacter.add(legR);
  ...
  if (s === 'short') {
    ...
    refs.previewCharacter.add(cap3);
    addOutline(cap3, 1.1);
  ...
  eL.position.set(-0.18, 2.05, 0.48);
  refs.previewCharacter.add(eL);

  const eR = eL.clone();
  eR.position.x = 0.18;
  refs.previewCharacter.add(eR);
  ...
  // Cara
  const showSmile = (
    appearance.face === 'smile' ||
    appearance.face === 'all' ||
    appearance.face === 'freckles' ||
    appearance.face === 'blush'
  );
...
// ============================================================
// ANIMACION DE LA VISTA PREVIA (gira al personaje)
// ============================================================
export function animatePreview() {
  if (!refs.previewRenderer) return;
  requestAnimationFrame(animatePreview);

  if (refs.previewCharacter) {
    refs.previewCharacter.rotation.y += 0.01;
  }
  refs.previewRenderer.render(refs.previewScene, refs.previewCamera);
}

// ============================================================
// CONSTRUIR LOS BOTONES DE OPCIONES DEL VESTIDOR
// ============================================================
